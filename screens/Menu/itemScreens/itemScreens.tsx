...
import { getAllItemsByType, searchItem } from "../../../api/item";
...
export default function ItemScreens({ navigation }: { navigation: any }) {
  ...
  async function prepare(isRefreshing: boolean = false) {
    ...
    const itemResponse = await getAllItemsByType(itemType.current);
        ...
        <FlatList
          ...
          ListHeaderComponent={
            <View style={{ marginBottom: 10 }}>
              {searchContainer(formik, buttonVisible, apiInUse, "itemName")}
              <ScrollView
                horizontal
